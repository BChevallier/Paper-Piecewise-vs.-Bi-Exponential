...
Reads `treadmill/data/raw/time_trial.csv` — a wide table, one row per
5-second sample and one column per participant, VO2 in L/min — and writes
`treadmill/data/prepared/time_trial.csv`, keeping only the rows
...
Run from anywhere, e.g. `python treadmill/scripts/02_prepare_time_trial_data.py`
...
from treadmill_common import RAW_TIME_TRIAL as RAW_PATH
from treadmill_common import TIME_TRIAL as OUTPUT_PATH
