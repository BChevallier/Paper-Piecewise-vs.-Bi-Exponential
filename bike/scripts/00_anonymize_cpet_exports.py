...
Usage:
    python bike/scripts/00_anonymize_cpet_exports.py            # blank the fields in place
    python bike/scripts/00_anonymize_cpet_exports.py --check    # report only, change nothing
"""
...
CPET_EXPORTS = REPO_ROOT / "bike" / "data" / "raw" / "cpet_exports"
...
def main(check_only):
    paths = sorted(CPET_EXPORTS.glob("*.xml"))
    if not paths:
        raise SystemExit(f"No XML files found in {CPET_EXPORTS}")
