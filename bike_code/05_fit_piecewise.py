...
`code/vo2_modeling/piecewise_model.py`): for every participant and every
...
- `<trial>_breakpoints_piecewise.csv` — breakpoint (s) x participant x method
- `<trial>_breakpoints_piecewise_rmse_pct.csv` — RMSE% x participant x method

Usage:
    python bike_code/05_fit_piecewise.py        # both trials
    python bike_code/05_fit_piecewise.py tt1    # one trial
"""
...
def run(trial):
    ...
    breakpoints_path = RESULTS_DIR / f"{trial}_breakpoints_piecewise.csv"
    rmse_path = RESULTS_DIR / f"{trial}_breakpoints_piecewise_rmse_pct.csv"
